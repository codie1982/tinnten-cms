...
export default function CronListsPage() {
  ...
  // Kaynak değişince alanlar/ilişkiler farklı → filtre/ilişkileri sıfırla.
  ...
  return (
    <RoleGuard allowedRoles={[CMS_ROLES.EDITOR]}>
      <PageHeader
        section="Email · Cron Listeleri"
        title="Cron Listeleri"
        description="DB sorgusundan otomatik mail listesi üret — zamanlanmış. Sonuç kanalları Kampanyalar'da hedef olarak görünür."
        actions={
          ...
          <CardContent className="space-y-4 p-4">
            ...
            <div className="flex flex-wrap gap-3">
              <div className="min-w-[200px]">
                ...
                <select className={SELECT_CLS} value={CRON_PRESETS.find((p) => p.cron === form.schedule.cron)?.cron || ''}
                ...
                <tbody>
                  {lists.map((row) => (
                    <tr key={row._id} className="border-b last:border-0 hover:bg-muted/30">
                      ...
                      <td className="p-3 font-mono text-xs">{row.schedule?.cron} <span className="text-muted-foreground">({row.schedule?.timezone})</span></td>
                      ...
                      <td className="p-3"><Badge variant={row.status === 'active' ? 'secondary' : 'outline'}>{row.status}</Badge></td>
                      <td className="p-3">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="sm" title="Şimdi çalıştır" onClick={() => onRun(row)}><Play className="size-3.5" /></Button>
                          <Button variant="ghost" size="sm" title="Düzenle" onClick={() => openEdit(row)}><RefreshCw className="size-3.5" /></Button>
                          <Button variant="ghost" size="sm" title="Sil" onClick={() => onDelete(row)}><Trash2 className="size-3.5" /></Button>
                        </div>
                      </td>
                    </tr>
                  ))}
