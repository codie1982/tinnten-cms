...
import { AI_ROLES, AI_PROVIDERS, aiCatalogApi as api, canWriteCatalog, catalogError, isDecimal } from '@/lib/ai-catalog';
...
function Catalog() {
  ...
  useEffect(() => {
    ...
    (async () => { const ctx = await api.context(abort.signal); if (abort.signal.aborted) return; setContext(ctx); if (!ctx.availability.catalog) return; const page = await api.models(abort.signal, cursor); if (!abort.signal.aborted) { setModels(page.items); setNextCursor(page.nextCursor); } })()
  ...
  return <div className="space-y-5">
    ...
    {!loading && !context?.availability.catalog && <div role="status" className={`${panel} text-sm`}><strong>Backend entegrasyonu bekleniyor</strong><p className="mt-2 text-muted-foreground">Katalog ve fiyat sözleşmeleri hazır. Servis etkinleşene kadar yazma/yayın işlemleri kapalıdır. Ekranda örnek model veya fiyat gösterilmez.</p></div>}
    ...
    {form && <form onSubmit={create} className={`${panel} grid gap-4 md:grid-cols-2`}><h2 className="font-semibold md:col-span-2">Yeni global model taslağı</h2>
      ...
      <Field label="Sağlayıcı"><select className={control} value={draft.provider} onChange={e => setDraft({ ...draft, provider: e.target.value, connectionTypes: [e.target.value] })}>{AI_PROVIDERS.map(p => <option key={p}>{p}</option>)}</select></Field>
    ...
    <section className={`${panel} space-y-4`}><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="flex items-center gap-2 font-semibold"><Database size={18} />Merkezi modeller</h2><Field label="Bu sayfada ara"><input type="search" className={control} value={query} onChange={e => setQuery(e.target.value)} /></Field></div>
      ...
      {!loading && context?.availability.catalog && !error && models.length === 0 && <p className="py-6 text-center text-sm text-muted-foreground">Henüz global model yok.</p>}
