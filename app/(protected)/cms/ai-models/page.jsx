...
import { AI_ROLES, AI_PROVIDERS, AI_CONNECTION_TYPES, aiCatalogApi as api, canWriteCatalog, catalogError, isDecimal, isCreditRate } from '@/lib/ai-catalog';
...
const EMPTY_PRICE = { kind: 'tinnten_tariff', input: '', output: '', effectiveAt: '', sourceUrl: '' };
...
function Catalog() {
  ...
  function addPrice(event) {
    event.preventDefault(); const validRate = price.kind === 'tinnten_tariff' ? isCreditRate : isDecimal;
    if (!selected || !validRate(price.input) || !validRate(price.output)) return;
    const { effectiveAt, sourceUrl, ...fields } = price;
    if (!effectiveAt) return;
    void perform(() => api.price(context, selected.id, { ...fields, ...(sourceUrl ? { sourceUrl } : {}), currency: price.kind === 'tinnten_tariff' ? 'CREDIT' : 'USD', unit: 'million_tokens', effectiveAt: new Date(effectiveAt).toISOString(), expectedVersion: selected.version }), () => { setSelected(null); setPrice(EMPTY_PRICE); });
  }
  return <div className="space-y-5">
    ...
    <div className={`${panel} flex flex-wrap justify-between gap-3`}><p className="flex items-center gap-2 text-sm text-muted-foreground"><ShieldCheck size={18} />Yalnız Tinten CMS yöneticileri. Firma model listelerinden bağımsızdır.</p><button className={button} onClick={() => setForm(!form)}><Plus size={16} />{form ? 'Formu kapat' : 'Model tanımı'}</button></div>
    {!loading && !context?.availability.catalog && <div role="status" className={`${panel} text-sm`}><strong>Backend entegrasyonu bekleniyor</strong><p className="mt-2 text-muted-foreground">Aktarım taslakları servis etkinse aşağıda DB’den listelenir. DigitalOcean bir sağlayıcı, Tinten ise yönetimli bağlantı türüdür. Başlangıç kredi tarifeleri llmmodels dosyasındaki kullanıcı tarafından seçilen fiyat referansından alınır. CMS yazma/fiyat/yayın işlemleri henüz kapalıdır.</p></div>}
    ...
    <section className={`${panel} space-y-4`}><h2 className="font-semibold">{selected ? `${selected.name} · Yeni fiyat sürümü` : 'Model / fiyat sürümü formu'}</h2><p className="text-sm text-muted-foreground">Tinten tarifesi doğrudan kredi / 1 milyon token olarak girilir; dolar veya kur alanı yoktur. Başlangıç tarifeleri llmmodels dosyasından alınır. Sağlayıcı maliyet tahmini ayrıdır; yeni fiyat geçmiş kullanımı yeniden hesaplamaz.</p>
      <form onSubmit={addPrice} className="grid gap-4 md:grid-cols-3"><Field label="Fiyat türü"><select className={control} value={price.kind} onChange={e => setPrice({ ...price, kind: e.target.value, input: '', output: '' })}><option value="provider_estimate">Sağlayıcı maliyet tahmini</option><option value="tinnten_tariff">Tinten tahakkuk tarifesi</option></select></Field>
        <Field label={price.kind === 'tinnten_tariff' ? 'Input · kredi / 1M token' : 'Input · USD / 1M token'}><input className={control} inputMode="decimal" required pattern="(0|[1-9][0-9]*)(\.[0-9]+)?" value={price.input} onChange={e => setPrice({ ...price, input: e.target.value })} /></Field>
        <Field label={price.kind === 'tinnten_tariff' ? 'Output · kredi / 1M token' : 'Output · USD / 1M token'}><input className={control} inputMode="decimal" required pattern="(0|[1-9][0-9]*)(\.[0-9]+)?" value={price.output} onChange={e => setPrice({ ...price, output: e.target.value })} /></Field>
        <Field label="Geçerlilik başlangıcı"><input type="datetime-local" className={control} required value={price.effectiveAt} onChange={e => setPrice({ ...price, effectiveAt: e.target.value })} /></Field><Field label="Fiyat kaynağı (HTTPS)"><input type="url" className={control} required={price.kind === 'provider_estimate'} pattern="https://.*" value={price.sourceUrl} onChange={e => setPrice({ ...price, sourceUrl: e.target.value })} /></Field>
        <button className={primary} disabled={!selected || !writable || !(price.kind === 'tinnten_tariff' ? isCreditRate : isDecimal)(price.input) || !(price.kind === 'tinnten_tariff' ? isCreditRate : isDecimal)(price.output)}>Fiyat sürümü oluştur</button>
      </form>
    </section>
  </div>;
}
